              ...
              <tr>
                {
                  footer.map((item) => {
                    ...
                    return (<td colSpan={item.colspan || 1} className={`Total ${item.hide ? 'Hide' : 'Show'}`} style={item.style || {}} key={item.key}>{renderHTML(renderText(item.label || total[item.key].toLocaleString()))}</td>)
                  })
                }
